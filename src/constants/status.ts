...
export function leaseSteps (captions: Partial<Record<LeaseStatus, string>> = {}): LifecycleStep[] {
  return (['PENDING', 'ACTIVE', 'COMPLETED'] as const).map(key => ({
    key,
    label: LEASE_STATUS[key].label,
    icon: key === 'PENDING' ? 'mdi-file-document-edit-outline' : LEASE_STATUS[key].icon,
